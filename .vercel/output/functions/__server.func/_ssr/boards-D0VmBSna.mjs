...
//#region node_modules/.nitro/vite/services/ssr/assets/boards-D0VmBSna.js
function hint(token) {
...
var getBoardPublic = createServerFn({ method: "GET" }).validator((input) => input).handler(getBoardPublic_createServerFn_handler, async ({ data }) => {
	...
	const crewRows = ids.length === 0 ? [] : await sql.query(`select cs.deal_id, m.name, cs.role
             from crew_shifts cs
             join members m on m.id = cs.member_id
             where cs.deal_id in (${ids.join(",")})
             order by cs.starts_at`).catch(() => []);
	const crewByDeal = /* @__PURE__ */ new Map();
