...
import { r as consumeCredit } from "./governance-Bb-_vds7.mjs";
import { r as hashSha } from "./access-BE7LFNbd.mjs";
import { t as createServerRpc } from "./createServerRpc-CcvdN_gc.mjs";
import { c as insertOutbound } from "./domain-DTp-hrMr.mjs";
import { l as mapOrg, n as mapActivity, r as mapDeal, t as DEAL_SELECT, u as mapPerson } from "./map-CMh3BgSg.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/ultimate-Dj5tMCOu.js
var NYC = {
	Finance: {
		lat: 40.7616,
		lng: -73.969,
...
var enrollSequence = createServerFn({ method: "POST" }).middleware([authMiddleware]).validator((input) => input).handler(enrollSequence_createServerFn_handler, async ({ data }) => {
	...
	if (seq && person?.email) {
		let steps = [];
		try {
			steps = typeof seq.steps === "string" ? JSON.parse(String(seq.steps)) : seq.steps ?? [];
		} catch {
			steps = [];
		}
...
var runAutomation = createServerFn({ method: "POST" }).middleware([authMiddleware]).validator((input) => input).handler(runAutomation_createServerFn_handler, async ({ data }) => {
	...
	if (String(a.action_type) === "email.template") {
		const tpl = (await sql.query(`select * from email_templates where name ilike '%' || $1 || '%' or $1 ilike '%' || name || '%' order by id limit 1`, [String(a.action_detail ?? "COI")]))[0] ?? (await sql.query(`select * from email_templates order by id limit 1`))[0];
		const person = (await sql.query(`select p.email, p.name, d.id as deal_id from deals d join people p on p.id = d.person_id
           where d.status = 'open' and p.email is not null order by d.id limit 1`))[0];
		if (tpl && person) {
			const first = String(person.name).split(" ")[0] ?? "there";
			const body = String(tpl.body).replaceAll("{{first_name}}", first).replaceAll("{{venue}}", "the venue").replaceAll("{{deal}}", String(a.name));
			await insertOutbound(sql, {
				purpose: "workflow",
