...
//#region node_modules/.nitro/vite/services/ssr/assets/governance-WUzBmkB-.js
var DEFAULT_POLICY = {
	hoursEnforced: true,
	officeStart: "07:00",
	officeEnd: "22:00",
...
var sendBroadcast = createServerFn({ method: "POST" }).middleware([authMiddleware]).validator((input) => input).handler(sendBroadcast_createServerFn_handler, async ({ data }) => {
	...
	const sql = await getSql();
	let recipients = [];
	if (data.audience === "leads") recipients = (await sql`select coalesce(p.name, l.title) as name, p.email from leads l left join people p on p.id = l.person_id where l.status <> 'archived' and p.email is not null`).map((r) => ({
		name: String(r.name),
		email: String(r.email)
	}));
	else if (data.audience === "rotting") recipients = (await sql`select coalesce(p.name, d.title) as name, p.email from deals d left join people p on p.id = d.person_id
          join stages s on s.id = d.stage_id
          where d.status = 'open' and p.email is not null
            and extract(day from now() - d.stage_entered_at) >= s.rotting_days`).map((r) => ({
		name: String(r.name),
		email: String(r.email)
	}));
	else recipients = (await sql`select coalesce(p.name, d.title) as name, p.email from deals d left join people p on p.id = d.person_id
          where d.status = 'open' and p.email is not null`).map((r) => ({
		name: String(r.name),
		email: String(r.email)
	}));
	const unique = [...new Map(recipients.map((r) => [r.email, r])).values()].slice(0, 40);
	for (const r of unique) await sql`insert into emails (folder, from_name, from_addr, to_addr, subject, body, opened, clicked, sent_at)
