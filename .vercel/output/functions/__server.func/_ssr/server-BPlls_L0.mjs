...
import { n as assertCap, u as guardAction } from "./governance-Bb-_vds7.mjs";
import { t as createServerRpc } from "./createServerRpc-CcvdN_gc.mjs";
import { c as insertOutbound } from "./domain-DTp-hrMr.mjs";
import { a as mapDoc, c as mapMember, d as mapProduct, f as mapProject, i as mapDealProduct, l as mapOrg, n as mapActivity, o as mapEmail, p as mapTask, r as mapDeal, s as mapLead, t as DEAL_SELECT, u as mapPerson } from "./map-CMh3BgSg.mjs";
import { t as completeNewBooking } from "./schedule-DBuILO8w.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/server-BPlls_L0.js
...
var getBootstrap = createServerFn({ method: "GET" }).handler(getBootstrap_createServerFn_handler, async () => {
	...
	try {
		eventTypes = (await sql`select * from event_types where active = true order by sort_order`).map((r) => ({
			id: Number(r.id),
			name: String(r.name),
			slug: String(r.slug),
			sortOrder: Number(r.sort_order),
...
var createDeal = createServerFn({ method: "POST" }).validator((input) => input).handler(createDeal_createServerFn_handler, async ({ data }) => {
	const rows = await (await getSql())`insert into deals (title, value, pipeline_id, stage_id, owner_id, org_id, person_id, venue, event_date, guest_count, source, event_type)
...
var setDealStatus = createServerFn({ method: "POST" }).validator((input) => input).handler(setDealStatus_createServerFn_handler, async ({ data }) => {
	...
	} else await sql`update deals set status = 'open', lost_reason = null, won_at = null, lost_at = null, cancelled_at = null, updated_at = now() where id = ${data.id}`;
...
var updateDeal = createServerFn({ method: "POST" }).validator((input) => input).handler(updateDeal_createServerFn_handler, async ({ data }) => {
	...
	await sql`update deals set
      ...
      event_type = ${data.eventType === void 0 ? cur.event_type : data.eventType},
      updated_at = now()
      where id = ${data.id}`;
	return { ok: true };
});
...
var updateLead = createServerFn({ method: "POST" }).validator((input) => input).handler(updateLead_createServerFn_handler, async ({ data }) => {
	...
	await sql`update leads set
      status = ${nextStatus},
      owner_id = ${data.ownerId ?? cur.owner_id},
      score = ${data.score ?? Number(cur.score)},
      event_type = ${data.eventType === void 0 ? cur.event_type : data.eventType},
      disqualify_reason = ${data.disqualifyReason === void 0 ? cur.disqualify_reason : data.disqualifyReason},
      stage_entered_at = ${statusChanged ? (/* @__PURE__ */ new Date()).toISOString() : cur.stage_entered_at}
      where id = ${data.id}`;
