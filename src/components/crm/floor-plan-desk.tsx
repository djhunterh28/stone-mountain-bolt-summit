...
export function FloorPlanDesk({
  ...
  const desk = useQuery({ queryKey: ["floors", dealId], queryFn: () => getFloorPlans({ data: { dealId } }) });
  ...
  const library = plans.filter((p) => p.template && p.dealId == null);
  const plots = plans.filter((p) => p.dealId === dealId);
  const plan = plots.find((p) => p.id === planId) ?? (planId == null ? plots[0] ?? null : null);
  ...
  function writeCache(next: FloorPlan[]) {
    qc.setQueryData(["floors", dealId], next);
  }

  function refresh() {
    void qc.invalidateQueries({ queryKey: ["floors", dealId] });
  }
  ...
  async function onDuplicate(fromId?: number) {
    const source = fromId ?? plan?.id;
    if (!source) return;
    const res = await duplicateFloorPlan({ data: { id: source, dealId } });
    if (res.ok && res.plan) {
      writeCache([res.plan, ...plans.filter((p) => p.id !== res.plan!.id)]);
      setPlanId(res.plan.id);
      toast.success("Plot copied onto this deal");
      refresh();
    }
  }

  async function onCreate() {
    const made = await createFloorPlan({
      data: {
        name: title ? `${title} plot` : "Show plot",
        venue: dealVenue ?? venue ?? "",
        dealId,
      },
    });
    writeCache([made, ...plans.filter((p) => p.id !== made.id)]);
    setPlanId(made.id);
    toast.success("Plot linked to this deal");
    refresh();
  }
  ...
  async function onArchive() {
    ...
    toast.success("Plot removed from this deal");
    setPlanId(null);
    refresh();
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Linked to this deal. Copy a venue shell or start a blank plot — it stays on this show only.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <Stat label="This deal" value={String(plots.length)} hint="Plots linked to this show" />
      ...
      <div className="grid gap-4 lg:grid-cols-[16rem_minmax(0,1fr)_16rem]">
        <aside className="space-y-4">
          {desk.isLoading ? (
            ...
            <>
              <PlanGroup title="This deal" items={plots} activeId={plan?.id} onPick={setPlanId} empty="New plot or copy a library shell onto this show." />
              <PlanGroup
                title="Venue library"
                items={library}
                onPick={(id) => void onDuplicate(id)}
                empty="No reusable shells yet."
              />
            </>
          )}
        </aside>
        ...
        <section>
          ...
          {desk.isLoading ? (
            ...
            <div className="rounded-xl bg-card px-4 py-16 text-center text-sm text-muted-foreground shadow-[var(--shadow-border)]">
              No plot on this deal yet. Start a blank plot or copy a venue from the library.
            </div>
          )}
