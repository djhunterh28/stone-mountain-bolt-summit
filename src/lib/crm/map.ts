...
export function mapDeal(r: Record<string, unknown>): DealCard {
  const days = Number(r.days_in_stage ?? 0);
  const rottingDays = Number(r.rotting_days ?? 14);
  const status = (r.status as DealCard["status"]) ?? "open";
  return {
    id: Number(r.id),
    title: String(r.title),
    value: money(r.value),
...
export function mapActivity(r: Record<string, unknown>): Activity {
  return {
    id: Number(r.id),
    type: String(r.type),
...
export function mapDealProduct(r: Record<string, unknown>): DealProduct {
  return {
    id: Number(r.id),
...
export function mapTask(r: Record<string, unknown>): ProjectTask {
  return {
    id: Number(r.id),
...
export function mapEmail(r: Record<string, unknown>): EmailRow {
  return {
    id: Number(r.id),
...
export function mapDoc(r: Record<string, unknown>): DocumentRow {
  return {
    id: Number(r.id),
    name: String(r.name),
    dealId: r.deal_id == null ? null : Number(r.deal_id),
