              ...
              {(tenants.data ?? []).map((t) => (
                <li key={t.id} className="flex flex-wrap items-center justify-between gap-2 px-4 py-3">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">{t.industry} · {t.slug}</div>
                  </div>
            ...
            {(tenants.data ?? []).map((t) => (
              <div key={t.id} className="flex flex-wrap items-center gap-3 rounded-xl bg-card px-4 py-3 shadow-[var(--shadow-border)]">
                <div className="min-w-40 flex-1">
                  ...
                  <div className="mt-1 text-xs text-muted-foreground">{formatBytes(t.usedMb * 1_000_000)} of {t.quotaGb} GB</div>
                </div>
                <Input
                  className="w-24"
                  type="number"
            ...
            {(prefs.data ?? []).map((n) => (
              <label key={`${n.tenantId}-${n.kind}`} className="flex items-center justify-between rounded-lg bg-card px-4 py-3 text-sm shadow-[var(--shadow-border)]">
                ...
                <input
                  type="checkbox"
                  checked={n.enabled}
                  onChange={(e) => setNotifPref({ data: { tenantId: n.tenantId, kind: n.kind, enabled: e.target.checked } }).then(() => qc.invalidateQueries({ queryKey: ["notif-prefs"] }))}
                />
              </label>
            ))}
          </TabsContent>
